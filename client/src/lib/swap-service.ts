  ...
  async swapSolToGold(solAmount: number): Promise<SwapResult> {
    try {
      ...
      if (this.externalWallet && this.externalWallet.connected) {
        ...
        // Fallback: if external wallet balance seems too low, try self-contained
        if (currentBalance < solAmount) {
          const fallbackBalance = await selfContainedWallet.getBalance();
          console.log(`⚠️ External wallet balance (${currentBalance}) < swap amount (${solAmount})`);
          console.log(`🔄 Checking self-contained wallet balance: ${fallbackBalance} SOL`);
          if (fallbackBalance > currentBalance) {
            currentBalance = fallbackBalance;
            useExternalWallet = false;
            console.log(`✅ Using self-contained wallet balance instead: ${currentBalance} SOL`);
          }
        }
      } else {
      ...
      // Use same fee calculation as successful real send transactions
      const feeBuffer = 0.001; // Same as real-send-tab.tsx that works
      const requiredAmount = solAmount + feeBuffer;
      
      console.log(`💰 Balance check (same as real send):`);
      console.log(`  - Current balance: ${currentBalance.toFixed(6)} SOL`);
      console.log(`  - Swap amount: ${solAmount} SOL`);
      console.log(`  - Fee buffer: ${feeBuffer} SOL (same as real-send-tab)`);
      console.log(`  - Total required: ${requiredAmount.toFixed(6)} SOL`);
      console.log(`  - Remaining after swap: ${(currentBalance - requiredAmount).toFixed(6)} SOL`);
      console.log(`  - Balance sufficient: ${currentBalance >= requiredAmount ? '✅ YES' : '❌ NO'}`);
      
      if (currentBalance < requiredAmount) {
        console.error(`❌ Insufficient balance for swap`);
        const errorMsg = `Insufficient SOL balance for this transaction. Need ${requiredAmount.toFixed(6)} SOL but only have ${currentBalance.toFixed(6)} SOL`;
