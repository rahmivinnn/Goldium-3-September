...
export const AnimatedTokenomicsCharts: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Update every 2 minutes for real-time updates (reduced frequency)
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-white mb-2">Real-time Tokenomics Analytics</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-black border-white/10 hover:border-white/20 transition-all duration-300">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white/70 text-sm">Live GOLD Price</p>
                <p className="text-white text-lg font-bold">{formatCurrency(tokenomicsData.currentPrice)}</p>
                <div className={`flex items-center text-sm ${
                  tokenomicsData.priceChange24h >= 0 ? 'text-white' : 'text-white'
                }`}>
                ...
                <div className="text-xs text-white/60 mt-1">Real-time data</div>
              </div>
              <DollarSign className="w-8 h-8 text-white opacity-60" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-black border-white/10 hover:border-white/20 transition-all duration-300">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <BarChart3 className="w-8 h-8 text-white opacity-60" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-black border-white/10 hover:border-white/20 transition-all duration-300">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white/70/70 text-sm">Staking APY</p>
                <p className="text-white text-lg font-bold">{tokenomicsData.stakingAPY.toFixed(1)}%</p>
                <p className="text-white/70/50 text-xs">{formatNumber(tokenomicsData.totalStaked)} Staked</p>
              </div>
      ...
      <Card className="bg-black/70 border-white/20/40 hover:border-white/20/70 transition-all duration-300">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-white flex items-center gap-2">
              ...
              Real-time Analytics
              {isAnimating && <div className="w-2 h-2 bg-black rounded-full animate-pulse" />}
            </CardTitle>
            <div className="flex gap-2">
              <Button
                ...
                className={activeChart === 'price' ? 'bg-black text-black' : 'border-white/20/40 text-white/70 hover:bg-black/20'}
              >
                Price
              </Button>
              <Button
                ...
                className={activeChart === 'volume' ? 'bg-black text-black' : 'border-white/20/40 text-white/70 hover:bg-black/20'}
              >
                Volume
              </Button>
              <Button
                ...
                className={activeChart === 'staking' ? 'bg-black text-black' : 'border-white/20/40 text-white/70 hover:bg-black/20'}
              >
                Staking
              </Button>
            </div>
        ...
        <CardContent>
          <div className="relative">
            ...
            />
            <div className="absolute top-4 left-4 text-white/70/70 text-sm">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-white/70/70">Circulating Supply</span>
                <span className="text-white font-semibold">{formatNumber(tokenomicsData.circulatingSupply)}</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                <span className="text-white/70/70">Total Staked</span>
                <span className="text-white font-semibold">{formatNumber(tokenomicsData.totalStaked)}</span>
              </div>
              <div className="w-full bg-black/50 rounded-full h-2">
                <div 
                  className="bg-black h-2 rounded-full transition-all duration-1000"
              ...
              <div className="flex justify-between items-center">
                <span className="text-white/70/70">Your Balance</span>
                <span className="text-white font-semibold">{formatNumber(balances?.gold || 0)}</span>
              </div>
              <div className="w-full bg-black/50 rounded-full h-2">
                <div 
        ...
        <Card className="bg-black border-white/10 hover:border-white/20 transition-all duration-300">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="text-center p-3 bg-black/50 rounded-lg">
                <p className="text-white/70/70 text-sm">Total Supply</p>
                <p className="text-white text-lg font-bold">{formatNumber(tokenomicsData.totalSupply)}</p>
              </div>
            </div>
            
            <div className="space-y-2">
              <div className="flex justify-between">
                <span className="text-white/70/70">24h High</span>
                <span className="text-white">{formatCurrency(tokenomicsData.currentPrice * 1.15)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/70/70">24h Low</span>
                <span className="text-white">{formatCurrency(tokenomicsData.currentPrice * 0.85)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/70/70">All Time High</span>
